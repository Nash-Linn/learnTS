...
const Get = (url:string)=>{
  const fn:MethodDecorator = (target,key,desc:PropertyDescriptor)=>{
    ...
    //https://api.apiopen.top/api/getHaoKanVideo?page=0&size=10
    //获取到的数据是 res.data.result.list  所以这里需要传入一个_key
    //经过处理后 能直接通过 res.data 获取到 res.data.result 中的数据
...
const Result = ()=>{
  const fn :ParameterDecorator=(target,key,index)=>{
    ...
    Reflect.defineMetadata('key','result',target) //作用是给getList方法增加一个属性
  }
  return fn
}
